...
interface SelectionResult {
  selectedCharacters: CharacterAnalysis[];
...
export class CharacterSelectionService {
  ...
  async selectCharactersForStory(
    storyTitle: string,
    ...
    console.log(
      `[${requestId}][CharacterSelectionService] Starting AI character selection...`
    );

    // Fetch all available characters
    const allCharacters = await this.characterRepo.list(0, 1000);
    console.log(
      `[${requestId}] Found ${allCharacters.length} total characters in database`
    );

    if (allCharacters.length < 10) {
      throw new Error(
        `Not enough characters available. Found ${allCharacters.length}, need at least 10.`
      );
    }

    // Create AI prompt with story and character data
    const prompt = createCharacterSelectionPrompt(
      storyTitle,
      storyDescription,
      storyPlot,
      allCharacters.map((c) => ({
    ...
    console.log(
      `[${requestId}] Calling AI to analyze ${allCharacters.length} characters for story fit...`
    );
    ...
    try {
      result = JSON.parse(response) as SelectionResult;
    } catch (error) {
    ...
    if (result.selectedCharacters.length < 10) {
      console.error(
        `[${requestId}] AI selected only ${result.selectedCharacters.length} characters, need at least 10`
      );
      throw new Error(
        `AI selected insufficient characters (${result.selectedCharacters.length}/10 minimum)`
      );
    }

    if (result.selectedCharacters.length > 15) {
      console.warn(
        `[${requestId}] AI selected ${result.selectedCharacters.length} characters, trimming to 15`
      );
      result.selectedCharacters = result.selectedCharacters.slice(0, 15);
    }

    console.log(
      `[${requestId}] AI selected ${result.selectedCharacters.length} characters`
    );
    ...
    const selectedCharacterData = await this.characterRepo.findManyByIds(
      selectedIds
    );

    if (selectedCharacterData.length !== selectedIds.length) {
      console.warn(
        `[${requestId}] Some characters not found: requested ${selectedIds.length}, found ${selectedCharacterData.length}`
      );
    }

    console.log(
      `[${requestId}] Character selection successful: ${selectedCharacterData.length} characters retrieved`
    );
