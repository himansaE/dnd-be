...
import { generateStoryPrompts, createContinuationUserMessage, } from "../prompts/playStory.js";
...
export class StoryService {
    ...
    async initializePlayerProgress(playerId) {
    ...
    async executeActions(playerId, actions) {
        const progress = await this.getPlayerProgress(playerId);
        for (const action of actions) {
            switch (action.type) {
                case "GIVE_ITEM":
                    await this.addItemToInventory(progress.id, action.data.itemId, action.data.amount || 1);
                    break;
                case "START_QUEST":
                    await this.startQuest(progress.id, action.data.questId);
                    break;
                case "PROGRESS_QUEST":
                    await this.updateQuestProgress(progress.id, action.data.questId, action.data.amount || 1);
                    break;
                case "UPDATE_REPUTATION":
                    await this.updateReputation(progress.id, action.data.targetId, action.data.amount || 0);
                    break;
            }
        }
    }
    async addItemToInventory(progressId, itemId, amount) {
    ...
    async startQuest(progressId, questId) {
    ...
    async updateQuestProgress(progressId, questId, stepProgress) {
    ...
    async completeQuest(progressId, playerQuest) {
        // Update quest status
        await prisma.playerQuest.update({
            where: { id: playerQuest.id },
            data: {
    ...
    async updateReputation(progressId, faction, amount) {
    ...
    async generateStoryBase(title, description, plot) {
        const response = await createChat(generateCharacterGenerationPrompts(title, description, plot), {
            response_format: {
                type: "json_object",
            },
            meta: {
    ...
    async generateStoryStartScene(title, description, plot, base, selectedCharacters) {
        let content;
        let response = "";
        ...
        console.log(`[StoryService.generateStoryStartScene] Passing ${charactersForAI.length} characters to AI`);
        const originalPrompts = generateStoryPrompts(title, description, plot, base.scene, JSON.stringify(charactersForAI));
        let retryCount = 0;
        const maxRetries = 5;
        while (retryCount < maxRetries) {
            try {
                if (retryCount === 0) {
                    ...
                    console.warn(`Retry attempt ${retryCount + 1}/${maxRetries} to regenerate JSON...`);
                    await new Promise((resolve) => setTimeout(resolve, 1500 * retryCount));
                    const regenerationPrompts = [
                        ...originalPrompts,
                        {
                            role: "user",
                            content: "The previous response was incomplete or malformed JSON. Please provide the **complete and valid JSON object** containing the Ink code again.",
                        },
                    ];
                ...
                console.log(`Successfully parsed JSON after ${retryCount + 1} attempts.`);
                break;
            }
            catch (error) {
                console.error(`JSON parsing or API call failed on attempt ${retryCount + 1}:`, error.message);
                console.log("Problematic response content:", response);
                retryCount++;
                if (retryCount >= maxRetries) {
                    console.error(`Max retries (${maxRetries}) reached. Failed to get valid JSON.`);
                    // If we ran out of retries, throw an error to indicate failure
                    throw new Error("Failed to generate story scene due to persistent JSON errors after retries.");
                }
            }
        }
        if (!content || typeof content !== "object" || content === null) {
            console.error("Final result does not contain the expected valid JSON structure with an 'ink' string property.", content);
            throw new Error("Generated content is missing the expected JSON structure or 'ink' property.");
        }
        return content;
    }
    async startStory(title, description, plot, characterIds) {
        console.log(`[StoryService.startStory] START - Fetching ${characterIds.length} characters`);
        // Fetch selected characters from database
        const characters = await this.characterRepo.findManyByIds(characterIds);
        console.log(`[StoryService.startStory] Fetched ${characters.length} characters:`, characters.map((c) => ({ id: c.id, name: c.name })));
        if (characters.length < 10 || characters.length > 30) {
            throw new Error(`Expected 10-30 characters, got ${characters.length}. Some character IDs may be invalid.`);
        }
        ...
        const startScene = await this.generateStoryStartScene(title, description, plot, storyBase, characters);
    ...
    async continueStory(conversationHistory, currentSegmentId, choiceId, nextSegmentId, flowHistory) {
        try {
    ...
    async generateStoryContinuation(conversationHistory, currentSegmentId, choiceId, nextSegmentId, flowHistory) {
        ...
        while (retryCount < maxRetries) {
            try {
                if (retryCount === 0) {
                    console.log(`Attempting story continuation from segment ${currentSegmentId} to ${nextSegmentId}...`);
                    ...
                    console.warn(`Retry attempt ${retryCount + 1}/${maxRetries} to regenerate continuation JSON...`);
                    await new Promise((resolve) => setTimeout(resolve, 1500 * retryCount));
                    const regenerationPrompts = [
                        ...fullConversation,
                        {
                            role: "user",
                            content: "The previous response was incomplete or malformed JSON. Please provide the **complete and valid JSON object** containing the continuation segments again.",
                        },
                    ];
                    response = await createChat(regenerationPrompts, {
                        response_format: {
                ...
                const sanitized = this.sanitizeContinuationContent(content, flowHistory ?? [], nextSegmentId);
                ...
                content = sanitized;
                console.log(`[generateStoryContinuation] After assignment to content:`, {
                    hasSegments: !!content?.segments,
                    hasNestedSegments: !!content?.segments?.segments,
                ...
                if (!content?.segments || !content.segments[nextSegmentId]) {
                    console.error(`VALIDATION FAILED: AI did not generate the requested segment "${nextSegmentId}"`);
                    console.error(`Available segment IDs:`, Object.keys(content?.segments ?? {}));
                    // If this is not the last retry, try again
                    if (retryCount < maxRetries - 1) {
                        console.warn(`Will retry (attempt ${retryCount + 2}/${maxRetries})...`);
                        retryCount++;
                        continue;
                    }
                    else {
                        throw new Error(`AI failed to generate the requested segment "${nextSegmentId}" after ${maxRetries} attempts. Available segments: ${Object.keys(content?.segments ?? {}).join(", ")}`);
                    }
                }
                console.log(`Story continuation parsed successfully after ${retryCount + 1} attempts. Requested segment "${nextSegmentId}" is present.`);
                break;
            }
            catch (error) {
                console.error(`JSON parsing or API call failed on attempt ${retryCount + 1}:`, error.message);
                ...
                retryCount++;
                if (retryCount >= maxRetries) {
                    console.error(`Max retries (${maxRetries}) reached. Failed to get valid continuation JSON.`);
                    throw new Error("Failed to generate story continuation due to persistent JSON errors after retries.");
                }
            }
        }
        if (!content || typeof content !== "object" || content === null) {
            console.error("Final result does not contain the expected valid JSON structure with segments.", content);
            throw new Error("Generated content is missing the expected JSON structure or segments.");
        }
    ...
    sanitizeContinuationContent(raw, previousIds, requiredId) {
        ...
        if (raw?.segments?.segments && typeof raw.segments.segments === "object") {
            ...
            console.log("[sanitizeContinuationContent] Detected nested segments.segments structure, flattening...");
        ...
        const result = {};
        const normalizeNarrativeItem = (item) => {
        ...
        for (const key of Object.keys(segmentsObj)) {
            ...
            if (previous.has(key) && key !== requiredId)
                continue;
