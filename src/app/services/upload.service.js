...
export class UploadService {
    ...
    async putObject(params) {
        ...
        console.log(`[UploadService.putObject] START - key: ${key}, contentType: ${params.contentType}`);
        console.log(`[UploadService.putObject] Body type: ${typeof params.body}, size: ${params.body instanceof Buffer ? params.body.length : "unknown"}`);
        console.log(`[UploadService.putObject] Bucket: ${this.bucket}, Endpoint: ${this.endpoint}`);
        try {
            console.log(`[UploadService.putObject] Sending PutObjectCommand to R2...`);
            ...
            if (base) {
                ...
                try {
                    ...
                    console.warn(`[UploadService.putObject] Failed to parse endpoint URL:`, e);
                }
            }
            else {
                console.warn(`[UploadService.putObject] No BUCKET_URL or endpoint, URL is just the key: ${url}`);
            }
            console.log(`[UploadService.putObject] SUCCESS - returning { key: ${key}, url: ${url} }`);
            return { key, url };
        }
        catch (error) {
            ...
            console.error(`[UploadService.putObject] Stack:`, error?.stack);
            throw error;
        }
    }
}
